...
def get(key: str, dic: dict, default=False):
    """
...
def add_merge_count_to_value(temp: dict, result: dict) -> dict:
    """ Adds value count, merging dicts together """
...
def remove_duplicates(key, hookups) -> list:
    unique = { each[key] : each for each in hookups }.values()
    result = []
    for key in unique:
        result.append(key)
    return result
